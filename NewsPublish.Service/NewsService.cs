...
namespace NewsPublish.Service
{
    public class NewsService
    {
        ...
        /// 新增一个新闻类别
        /// </summary>
        public ResponseModel AddNewClassify(AddNewsClassify newClassify)
        {
            var exit = _db.NewsClassify.FirstOrDefault(c => c.Name == newClassify.Name) != null;
            if (exit)
                return new ResponseModel { code = 0, result = "该类别已存在" };
            var classify = new NewsClassify { Name = newClassify.Name, Sort = newClassify.Sort, Remark = newClassify.Remark };
        ...
        {
            var news = _db.News.Include("NewsCalssify").Include("NewsComment").FirstOrDefault(c => c.Id == id);
            if (news == null)
        ...
        public ResponseModel GetRecommendNewsList(int newsId)
        {
            ...
            if (news == null)
                return new ResponseModel { code = 0, result = "新闻不存在" };
            var newsList = _db.News.Include("NewsComment").Where(c => c.NewsClassifyId == news.NewsClassifyId && c.Id != news.Id)
